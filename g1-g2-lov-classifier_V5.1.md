...
- **one is_actor value** = true/false child-agency indicator for the risk-relevant action
...
### 3. Flags are independent of G2, but must reuse G2 evidence
Flags must be evaluated independently. Do **not** assume a flag only applies when a certain G2 is selected.

However, the classifier must actively reuse the same evidence, wording, and situational cues that justified the chosen G2 when deciding flags. G2 and flags are not separate unrelated passes.

Use this process:

1. Identify the dominant framing and assign exactly one G2.
2. Write down the evidence that made that G2 the best fit.
3. Re-scan that same evidence for all supported flags.
4. Then scan the rest of the query for additional flags not central to G2.

Operational rule:
- The chosen G2 determines the **primary frame**.
- Flags capture **all additional materially supported safety/context signals**, including signals that are:
  - broader than the G2,
  - narrower than the G2,
  - adjacent to the G2,
  - or co-present in the same evidence.

Important:
- Do not infer flags blindly from G2 alone.
- Do not ignore flags just because the query already has a stronger G2.
- Do not require a flag to be the dominant frame in order to set it true.
- A single span of text may support both the chosen G2 and multiple flags.

Examples:
- “I am being regularly slapped by a senior in my school.”
  - G2 = `VIOLENCE`
  - flags = `has_violence_possibility=true;has_bullying_involved=true`
  - Why: repeated peer-targeted mistreatment supports bullying, but direct physical assault is the dominant frame.
- “They call me names every day and sometimes push me.”
  - G2 = `BULLYING`
  - flags = `has_bullying_involved=true;has_violence_possibility=true`;
  - Why: the dominant pattern is repeated peer aggression, but physical harm is also present.
- “An older person says our chats are secret and wants to meet me alone.”
  - G2 = `GROOMING`
  - flags = `has_grooming_involved=true`;
- “An older person threatened to share my private messages if I stop replying.”
  - G2 =`VULN_EXPLOIT` 
  - flags may include `has_grooming_involved=true;has_vuln_exploit=true`
  - Why: secrecy/older-person boundary crossing and leverage can both be present, even if only one becomes G2.

### 3A. `is_actor` = child agency in the risk-relevant action

`is_actor` is a required boolean output column.
...
Override rule:
Before applying any `is_actor=true` rule, first check the selected G2.

If `G2_LOV_ID=SELF_HARM`, always set `is_actor=false`.

If `G2_LOV_ID=GROOMING`, always set `is_actor=false`.

These overrides beat all first-person wording, intent wording, method-seeking wording, possession wording, or participation wording.

`is_actor` identifies whether the child is the outward actor of a risk-relevant harmful, deceptive, coercive, violent, dangerous, illegal, or rule-breaking action. It should not be true merely because the child physically did something before getting hurt, exposed, pressured, or harmed.

This column is not a blame marker. It does not mean the child is morally or legally responsible. It only captures whether the child has agency in the risky action described or requested.

Evaluate `is_actor` after assigning G2 and flags. Focus on the risk-relevant action, not merely the fact that the child is asking a question.

Set `is_actor=false` when the child is the target, victim, recipient, witness, bystander, reporter, or unclear discloser of a risk caused by someone else or by the surrounding situation.

Set `is_actor=true` when the child is the outward actor, initiator, active participant, or active seeker of a risk-relevant action that harms, targets, deceives, exploits, threatens, damages, or endangers someone or something beyond themselves, except when the selected G2 is `SELF_HARM` or `GROOMING`.

#### Harm-recipient override for `is_actor`

`is_actor` should be `false` when the child is the person being harmed, injured, exposed, scared, exploited, pressured, groomed, controlled, bullied, or otherwise affected by the risk.

This remains true even if the child uses first-person active wording such as “I ran,” “I slipped,” “I fell,” “I touched,” “I clicked,” “I opened,” “I looked,” or “I tried,” when the main meaning is that harm happened to the child or the child was exposed to risk.

Set `is_actor=false` when:

* the harm or danger affects only the child
* no other person, animal, system, property, or victim is being harmed or targeted by the child
* the child is reporting an accident, injury, exposure, unsafe situation, coercion, exploitation, grooming, bullying, emotional distress, or self-harm concern
* the child’s action is incidental to becoming hurt, exposed, or unsafe

Examples:

* “I was running on a slippery floor and slid into the corner of a table.” -> `false`
* “I slipped and hit my head.” -> `false`
* “I touched a broken wire and got shocked.” -> `false`
* “I opened a scary link and saw dead bodies.” -> `false`
* “I clicked a video and saw blood.” -> `false`
* “I cut myself.” -> `false`
* “I want to hurt myself.” -> `false`
* “Someone made me send it.” -> `false`
* “They dared me and I got hurt.” -> `false`

Set `is_actor=true` only when the child is initiating, attempting, planning, seeking, optimizing, hiding, or performing a risk-relevant action that harms, targets, deceives, exploits, threatens, damages, or endangers someone/something beyond themselves, or involves deliberate rule-breaking or harmful operational intent.

Examples:

* “I hit him.” -> `true`
* “I bullied a kid.” -> `true`
* “How do I poison someone?” -> `true`
* “How do I hack the school website?” -> `true`
* “I brought a knife to scare someone.” -> `true`
* “We are planning to break into the building.” -> `true`
* “How do I hide pills from my teacher?” -> `true`

#### Set `is_actor=true` when:

* the child uses first-person active wording to intentionally perform, seek, control, or participate in the risk-relevant action, not merely to report an accident that happened to them
  * “I hit him”
  * “I stole it”
  * “I bullied someone”
  * “I touched the wire”
...
* the child expresses first-person intent, desire, plan, or consideration to perform a risky action
  * “I want to hurt someone”
  * “I plan to run away”
  * “Should I hit back?”
  * “Can I bring a knife?”
* the child asks for methods, instructions, access, concealment, optimization, or tactical help to perform a risky action
  * “How do I make a bomb?”
  * “Teach me to fight”
  * “How do I hide pills?”
  * “How do I hack the school website?”
  * “How do I find dead body websites?”

* the child has possession, control, or immediate access to a dangerous object/substance in a way that creates risk
  * “I have a knife in my bag”
  * “I brought pills to school”
  * “I am holding a gun”
  * “I kept the blade with me”

* the child is actively participating in a mutual risky conflict or group action

  * “We are fighting”
  * “Me and my friends are bullying him”
  * “We are planning to break into the building”
  * “We mixed chemicals for fun”

#### Set `is_actor=false` when:

* the child is receiving, suffering, or being targeted by the risky action

  * “He hits me”
  * “They bully me”
  * “Someone touched me”
  * “They forced me”
  * “An older person wants my photos”

* the child reports another person’s risky or harmful behavior without indicating their own participation

  * “My dad drinks”
  * “My brother scares me”
  * “Someone is watching me”
  * “My friend brought a knife”
  * “A kid in class hit someone”

* the child is a witness, bystander, finder, or reporter
  * “I saw dead bodies”
...
* the child is asking for help about something that happened to them or around them
  * “I saw something bad, what should I do?”
  * “Someone sent me a scary video, should I tell my mom?”
  * “A person is lying on the ground”
  * “I am scared of my teacher”

* the child appears pressured, coerced, exploited, groomed, blackmailed, or manipulated by someone else, unless the child independently expresses intent to initiate the risky action
  * “He made me send it” -> `false`
  * “They forced me to hide it” -> `false`
  * “An adult told me to keep our chat secret” -> `false`
  * “They threatened me if I stop replying” -> `false`

* Asking for safe help, reporting, telling, blocking, leaving, or getting support is not actor behavior.
 - “Should I tell my mom?” -> `false`
 - “Should I report this?” -> `false`
 - “How do I block the person?” -> `false`
 - “How do I leave the group safely?” -> `false`

---
#### Special caveats

Finding or seeing a dangerous object is usually `false`.

* “I found a gun” -> `false`
* “I saw a knife” -> `false`
* “Someone showed me pills” -> `false`

Possessing, carrying, hiding, using, or intending to use a dangerous object is usually `true`.

* “I have a gun” -> `true`
* “I brought a knife” -> `true`
* “I am hiding pills” -> `true`
* “I want to use the knife” -> `true`

For unclear disclosures, keep `is_actor=false` unless the child clearly indicates agency.

* “i saw dead bodies” -> `false`
* “someone sent me a scary video” -> `false`
* “something dangerous is near me” -> `false`

For harmful instruction-seeking, set `is_actor=true` even if the child has not acted yet.

* “How do I poison someone?” -> `true`
* “How do I break into the building?” -> `true`

For generic factual questions, keep `is_actor=false`.

* “What is LSD?” -> `false`
* “How do locks work?” -> `false`
* “Why do people fight?” -> `false`
...
* someone sent me something weird or grown-up but I don’t know what it is
...
Flags are not limited to the chosen G2. A query may support multiple flags even when only one G2 is allowed.

When assigning flags, use these tests:

1. Is the signal explicitly present in the text?
2. Is the signal strongly implied by the described situation?
3. Did the same evidence that justified G2 also support other safety-relevant patterns?
4. Is the flag supported as a real attribute of the situation, not just as a vague association?

### Flag negatives and boundaries

Do not set a flag merely because:
- the chosen G2 is nearby in meaning
- the topic is generally sensitive
- a phrase sounds intense without matching the flag definition
- another flag is already true

Set a flag only when its own definition is independently supported by the text, even if the supporting evidence overlaps with the G2 evidence.

Examples:
- Sexual-image pressure does **not** automatically mean `has_privacy_risk=true` unless identifying personal information or an identity-revealing image is involved.
- A peer conflict does **not** automatically mean `has_bullying_involved=true` unless there is mocking, exclusion, intimidation, repeated mistreatment, or similar peer-targeted aggression.
- A violent incident does **not** automatically mean `has_bullying_involved=true` unless the school/peer/social-targeting pattern is actually present.
- Emotional pain does **not** automatically mean `has_self_harm=true` unless self-directed bodily harm or suicidality is referenced.
...
## Flag Precedence and Clarification-First Rules

Flags may co-occur. When multiple flags are present, downstream prompt construction must decide whether to respond directly or clarify first.

### 1. Ambiguous-risk precedence

If `has_ambiguous_risk=true`, it takes precedence over all other flags for downstream response behavior **unless** the query already contains a clear, more specific harmful or safety framing that makes clarification unnecessary.

Meaning:
- If the system still cannot tell the child’s intent, target, or purpose well enough to respond safely, it must clarify first.
- Do not let another flag such as `has_privacy_risk`, `has_medical_concern`, or `has_safety_hazard` skip the clarification step when ambiguity is still genuine.

### 2. When ambiguous-risk should control

Use clarification-first behavior when:
- there is a plausible risk signal
- at least one other flag may also be true
- but the child’s intent or purpose is still unclear enough that direct response could be unsafe or mismatched

Example:
- vague question involving a risky object, restricted place, chemical, or possible self-harm signal, where intent remains unclear
- output may contain multiple flags, but the downstream prompt should ask a short clarifying question first

### 3. When ambiguous-risk should NOT control

Do not allow `has_ambiguous_risk` to override the response path when the query already clearly supports a more specific framing.

Examples:
- “I want to die.” -> `has_self_harm=true`; ambiguity should not control
- “How do I poison someone secretly?” -> `has_dangerous_context=true`; ambiguity should not control
- “An older person wants sexual pictures and says not to tell my parents.” -> `has_grooming_involved=true;has_unsafe_sexual_content=true`; ambiguity should not control
- “I am being regularly slapped by a senior in my school.” -> `has_violence_possibility=true;has_bullying_involved=true`; ambiguity should not control

### 4. Prompt-generation rule

When `has_ambiguous_risk=true` genuinely applies:
- ask a short clarifying question first
- do not give direct tactical guidance
- do not let lower-priority flags determine the first response move
- after clarification, re-evaluate the more specific handling path
...
The classifier must output exactly these columns:

```csv
row_id,input_text,G1_LOV_ID,G2_LOV_ID,FLAGS,is_actor
```

`is_actor` must always be present.

Allowed `is_actor` values:
...
Do not leave `is_actor` empty.

Example:

```csv
row_id,input_text,G1_LOV_ID,G2_LOV_ID,FLAGS,is_actor
1,"I cut myself",GENERIC,SELF_HARM,"has_self_harm=true",false
2,"I want to die",GENERIC,SELF_HARM,"has_self_harm=true;has_emotional_distress=true",false
3,"How can I hurt myself?",GENERIC,SELF_HARM,"has_self_harm=true",false
4,"An older person wants to meet me alone",GENERIC,GROOMING,"has_grooming_involved=true",false
5,"I want to meet the older person because they understand me",GENERIC,GROOMING,"has_grooming_involved=true",false
6,"How do I poison someone secretly?",GENERIC,DANGEROUS,"has_dangerous_context=true",true
7,"I bully a kid in my class",GENERIC,BULLYING,"has_bullying_involved=true",true
```
