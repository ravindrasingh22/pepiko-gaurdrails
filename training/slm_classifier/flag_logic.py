...
def apply_flag_logic(
    ...
    base = deepcopy(classifier_result)
    ...
    if not flags_trained:
        ...
        result["flags"] = flags
        result["flag_overrides"] = {
            "applied": False,
            "flag_trigger_threshold": FLAG_TRIGGER_THRESHOLD,
            "support_threshold": FLAG_SUPPORT_THRESHOLD,
            "added_to_g2_all": [],
            "primary_g2_before": str(result.get("g2", {}).get("id", "GENERIC_INTENT")),
            "primary_g2_after": str(result.get("g2", {}).get("id", "GENERIC_INTENT")),
            "primary_g2_overridden": False,
            "lexicon_matches": sorted(lexicon_matches),
            "flags_available": False,
            "reason": "Flag fusion skipped because the loaded checkpoint was not trained with the auxiliary flag head.",
        }
        result["logs"] = {"classifier": base}
        return result
    ...
    result["flag_overrides"] = {
        "applied": bool(added_labels or primary_overridden),
        "flag_trigger_threshold": FLAG_TRIGGER_THRESHOLD,
        "support_threshold": FLAG_SUPPORT_THRESHOLD,
        "added_to_g2_all": added_labels,
        "primary_g2_before": old_primary,
        "primary_g2_after": new_primary,
        "primary_g2_overridden": primary_overridden,
        "lexicon_matches": sorted(lexicon_matches),
